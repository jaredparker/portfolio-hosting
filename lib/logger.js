...
const fileFormat = winston.format.combine( winston.format.timestamp(), winston.format.printf( info => `[${info.timestamp}][${info.level}] ${info.message}` ) ); // winston.format.json()
...
const fileTransport = process.env.NODE_ENV == 'production' ? new winston.transports.DailyRotateFile({
    level: 'debug',
